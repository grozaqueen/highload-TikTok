...
1. **Упрощение рекомендаций**:  
   ...
   - Большая "учительская" модель обучает компактную "студенческую" модель (в 10-100 раз меньше). При нагрузке >80% CPU переключаем 50% трафика на slim-модель
1. **Поиск**:
   - Используем кэшированные векторные представления видео
   - Полностью отключаем векторный поиск - остается просто поиск по словам
   - отключаем Л2 ранжирование - используем только Л1
...
4. **Отложенная синхронизация соц. действий**:  
   - Лайки/комментарии могут сохраняться локально и синхронизироваться позже - Пользователь видит, что действие произошло сразу (лайк подсвечивается), даже если на сервер пока ничего не отправилось.
   - Лимит на действия (например, не более 10 лайков/мин с одного аккаунта) - Если пользователь превышает лимит, новые действия либо блокируются, либо откладываются.
   - Батчинг — объединение нескольких лайков в один запрос к БД - Это уменьшает нагрузку на сервер и экономит трафик.
5. **Очередь загрузки видео**:  
   - При высокой нагрузке загрузка новых видео замедляется - Видео загружается на сервер сразу, но обработка (например, конвертация в нужные форматы, создание превьюшек) происходит с небольшой задержкой.
