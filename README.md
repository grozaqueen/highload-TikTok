...
Используем session tickets для уменьшения количества полных обменов ключами, что снижает нагрузку на CPU. В среднем 4,7% запросов направляется на один дц, при этом в регионе работает 4 дц в среднем. При пиковом трафике 2,8 млн RPS на весь регион на 1 дц приходится 131k RPS. С учетом Session Tickets, 60% запросов обходят полноценное SSL-рукопожатие, а 40% (53000 RPS) требуют 2 round-trip'а. При средней задержке 2 мс: 2 мс × 53000 = 105 секунд процессорного времени в секунду на один дц. 
